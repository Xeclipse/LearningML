...
W = tf.Variable(tf.random_normal(shape = [n_input, 200]))
b = tf.Variable(tf.random_normal(shape =[200]))
layer_output = tf.nn.softmax(tf.matmul(net_input, W) + b)
W2= tf.Variable(tf.random_normal(shape =[200, n_output]))
b2=tf.Variable(tf.random_normal(shape =[n_output]))
net_output = tf.nn.softmax(tf.matmul(layer_output, W2) + b2)
