...
def gen_batch(raw_data, batch_size, num_steps):
    ...
    data_length = len(raw_x)  # 获取数据长度

    # 首先将数据切分成batch_size份，0-batch_size，batch_size-2*batch_size。。。
    batch_partition_length = data_length // batch_size  # 整数除法，将整个数据分成batch_size份，计算每一份的长度
    data_x = np.zeros([batch_size, batch_partition_length], dtype=np.int32)  # 生成一个batch_size*batch_partition_length的矩阵
    ...
    for i in range(batch_size):
        data_x[i] = raw_x[i * batch_partition_length:(i + 1) * batch_partition_length]  # 向矩阵中填充数据x
    ...
    epoch_size = batch_partition_length // num_steps  # 对于每一个batch，看分成多少个epoch，获取每个step的数据，垂直切分，即每次只拿一句话的一段出来
...
def gen_epochs(n, num_steps):
    '''这里的n就是训练过程中用的epoch，即在数据集规模上循环的次数'''
    for i in range(n):
...
x = tf.placeholder(tf.int32, [batch_size, num_steps])#每一次做的一段话的长度是num_steps
...
# n_class=2 是因为Y只有两种取值，0和1
x_one_hot = tf.one_hot(x, n_classes)#why,为什么要把x变成One_hot的形式呢？
